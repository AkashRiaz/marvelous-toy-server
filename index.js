...
async function run() {
  try {
    // Connect the client to the server	(optional starting in v4.7)
     client.connect();
     const toyCollection = client.db('actionToysDB').collection('toys')
     ...
     app.get('/allToy', async(req,res)=>{
        ...
        const sortedResult = result.sort((a, b) => sort * (parseFloat(a.price) - parseFloat(b.price)));
...
app.get('/',(req, res)=>{
    res.send('the server is running test')
})
